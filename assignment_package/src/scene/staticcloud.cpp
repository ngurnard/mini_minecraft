#include "staticcloud.h"

StaticCloud::StaticCloud(OpenGLContext* context, Noise* N)
...
GLenum StaticCloud::drawMode() {
    return GL_TRIANGLES;
}

void StaticCloud::createVBOdata() {
...
void StaticCloud::setHeight(int h) {
    m_height = h;
}

void StaticCloud::setThreshold(int t) {
    m_threshold = t;
}

void StaticCloud::setBounds(int xmin, int xmax, int zmin, int zmax) {
    m_bounds = glm::ivec4(xmin, xmax, zmin, zmax);
}

void StaticCloud::incrementOffset(glm::ivec2 increment) {
    m_offset += increment;
}
